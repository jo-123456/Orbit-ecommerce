...
## Deploying it for real (so you can share a live link)

This mirrors the same Render + Netlify pattern used for other projects:

**Backend → Render**
1. Push the `backend/` folder to GitHub.
2. On Render, create a **Web Service** from that repo, root directory `backend`.
3. Build command: `npm install`. Start command: `npm start`.
4. Add environment variables `MONGODB_URI`, `JWT_SECRET`, `PORT` in Render's dashboard (same values as your `.env`).

**Frontend → Netlify**
1. Push the `frontend/` folder to GitHub (or the whole repo, setting the base directory to `frontend`).
2. Build command: `npm run build`. Publish directory: `dist`.
3. Add an environment variable `VITE_API_URL` pointing to your deployed Render backend, e.g. `https://your-app.onrender.com/api`.
